...
for inv, dec in zip(st.session_state.active_invoices, decisions):
    with st.expander(f"View Rationale for {inv['id']} - {inv['supplier']}"):
        ...
        try:
            ...
            # Catch Gemini API rate limits without crashing the dashboard
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e) or "quota" in str(e).lower():
                explanation = (f"⚠️ **API Rate Limit Exceeded.** (Too many requests at once).\n\n"
                               f"**Local Rationale:** The ML selected '{dec['action'].replace('_', ' ').title()}' "
                               f"based on balancing a liquidity score of {dec['scores']['liquidity']} "
                               f"and a supplier impact score of {dec['scores']['supplier']}.")
            else:
