...
try:
    ...
    def explain_decision(decision: Dict[str, Any], invoice: Dict[str, Any]) -> str:
        """Returns a plain-English explanation"""
        return (f"The recommended action for Invoice {invoice['id']} is to '{decision['action']}'. "
                f"This decision is supported by a liquidity score of {decision['scores']['liquidity']} "
                f"and a supplier impact score of {decision['scores']['supplier']}. "
                f"Rationale: {decision['rationale']}")

    def detect_change(*args, **kwargs):
        """Mock monitor function to simulate dynamic real-world financial shifts"""
        pass

    def reoptimize(*args, **kwargs):
...
for inv, dec in zip(st.session_state.active_invoices, decisions):
    with st.expander(f"View Rationale for {inv['id']} - {inv['supplier']}"):
        ...
        explanation = explain_decision(dec, inv)
...
if st.button("Simulate Next Day", type="primary", use_container_width=True):
    ...
    detect_change(st.session_state.current_simulated_date)
    reoptimize(st.session_state.active_invoices)
